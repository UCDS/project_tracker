  <div class="row">
	...
	<div class="col-md-12">
		<div class="col-md-7">
			...
			<?php echo form_open('reports/staff',array('id'=>'work_type_form')); ?>
	...
	foreach($summary as $row){
		if(!in_array($row->staff_id,$staff)) {
			${"name_".$row->staff_id}=$row->staff_name;
			${"total_projects_".$row->staff_id}=0;
			${"not_started_".$row->staff_id}=0;
			${"work_in_progress_".$row->staff_id}=0;
			${"work_completed_".$row->staff_id}=0;
			${"work_load_".$row->staff_id}=0;
			${"expenses_current_".$row->staff_id}=0;
			${"pending_bills_".$row->staff_id}=0;
			$uds[]=$row->staff_id;
		}
	?>

	<?php
		if((count($divisions)>0 && !in_array($row->division_id,$divisions))){
			$division_id = $divisions[count($divisions)-1];
		//	echo "<script>console.log('".$division_id."');</script>";
				?>
		
				<tr onclick="$('#select_form_<?php echo $i;?>').submit();">
					<th colspan="3">Sub Total
						<?php echo form_open("reports/staff",array('id'=>'select_form_'.$i,'role'=>'form')); ?>
					<?php if($this->input->post('state')) { ?>
					<input type='hidden' value="<?php echo $this->input->post('state'); ?>" name="state" />
					<?php } ?>
					<input type='hidden' value="<?php if($division_id!=NULL) echo $division_id; else echo "'0'" ?>" name="<?php echo $id;?>" />
	...
	<tr onclick="$('#select_form_<?php echo $i;?>').submit();">
		...
		<input type='hidden' value="<?php if($row->staff_id!=NULL) echo $row->staff_id; else echo "'0'" ?>" name="<?php echo "staff_id";?>" />
		</td>
	...
	<?php
	if( $i==(count($summary) + count($divisions))){ 
				$division_id = $divisions[count($divisions)-1];
			echo "<script>console.log('".$division_id."');</script>";
				?>
	...
	$staff[]=$row->staff_id;
	${"total_projects_".$row->staff_id}+=$row->total_projects;
	${"not_started_".$row->staff_id}+=$row->not_started;
	${"work_in_progress_".$row->staff_id}+=$row->work_in_progress;
	${"work_completed_".$row->staff_id}+=$row->work_completed;
	${"work_load_".$row->staff_id}+=($row->tech_sanction_amount-$row->expenses_last_year);
	${"expenses_current_".$row->staff_id}+=($row->expenses_current_year+$row->expense_current_month);
	${"pending_bills_".$row->staff_id}+=$row->bills_pending;
	}
	?>
	</tbody>
	...
	<tr onclick="$('#select_form_<?php echo $i;?>').submit();">
		...
		<input type='hidden' value="<?php echo $this->input->post('state'); ?>" name="state" />
		<?php } ?>
		<input type='hidden' value="<?php if($ud!=NULL) echo $ud; else echo "'0'" ?>" name="staff_id" />
		<input type='hidden' value="<?php  echo $work_type;?>" name="<?php echo "work_type";?>" />
		</th>
		<th>
			<?php echo ${"name_".$ud}; ?>
